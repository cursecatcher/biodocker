...
void ConvertAlignment(const s_align& s_al,
  ...
  if (s_al.cigarLen > 0) {
    ...
    if (al->query_begin > 0) {
      al->sw_score=al->sw_score-al->query_begin;//MB to take into account soft clipping in the score
    ...
    if (end > 0) {
      al->sw_score=al->sw_score-end; //MB to take into account soft clipping in the score
